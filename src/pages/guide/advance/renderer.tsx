...
<p>Viewfly 采用自上而下的单向数据流方案，当父组件变化时，子组件也会自动渲染，同时，Viewfly 也会跳过数据没有数据变更的分支。总体来说，Viewfly 目前的 diff 算法是非常高效的。大部分场景我们并不需要关心性能问题。但总有一些场景，数据量特别大，变动又特别频繁。我们可以参考下面的策略进行性能调优。</p>
<h2>分支调优</h2>
<p>我们可以拆分变动频繁的部分和不常变动的部分为两个组件，并且让两个组件不属于父子或父孙关系，当 Viewfly 在更新 DOM 时，就会跳过没有变化的分支，这可以显著的提高 diff 的效率。</p>
<h2>Key</h2>
